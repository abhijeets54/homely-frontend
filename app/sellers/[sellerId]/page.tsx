                  ...
                  {filteredMenuItems.length === 0 ? (
                    ...
                    <AnimatedStaggerContainer className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {filteredMenuItems.map((item: any) => (
                        <AnimatedChild key={item._id}>
                          <motion.div
                            whileHover={item.isAvailable !== false && item.stock > 0 ? {
                              y: -5,
                              boxShadow: "0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 10px 10px -5px rgba(0, 0, 0, 0.04)"
                            } : {}}
                            transition={{ type: "spring", stiffness: 300, damping: 20 }}
                          >
                            <Card className={`overflow-hidden ${!item.isAvailable || item.stock <= 0 ? 'opacity-70' : ''}`}>
