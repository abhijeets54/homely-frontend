...
export function ImprovedCartSheet() {
  ...
  const handleOpenChange = useCallback((newOpen: boolean) => {
    // Only allow opening if it's not from initial page load
    if (newOpen && isPageLoad.current) {
      return;
    }
    setOpen(newOpen);
  }, []);
    ...
    <div>
      ...
      {open && (
        <div className="fixed inset-0 z-50 bg-black/80" onClick={() => handleOpenChange(false)}>
          <div 
            className="fixed inset-y-0 right-0 z-50 w-full sm:max-w-md md:max-w-lg flex flex-col h-full bg-background shadow-lg"
              ...
              <div className="mt-6 flex-1 flex flex-col min-h-0">
                {cartItems.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-12">
                    ...
                    <Button variant="outline" onClick={() => handleOpenChange(false)}>Continue Shopping</Button>
                  </div>
                ) : (
